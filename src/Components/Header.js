...
export default function Header() {
  ...
  const total = () => {
    let countPrice = 0;
    getdata.map((item) => {
      countPrice = item.price + countPrice;
    });
    setPrice(countPrice);
  }
                  ...
                  <tbody>
                    ...
                    <div className="d-flex flex-column mt-1">
                      <button className="btn btn-outline-success btn-sm mt-2" type="button">
                        <i class="fa-sharp fa-solid fa-cart-shopping"></i> <Link to={'/checkout'} style={{textDecoration:"none", color:"inherit"}} onClick={handleClose}>checkout</Link>  <span>&#8594;</span>
                      </button>
                    </div>
                  </tbody>
                </Table>
